...
var vlcJS = {
	...
	pl_play : function( id ) {
		...
		loadXMLDoc( 'requests/status.xml?command=pl_play' + id, parse_status );
		this.pl_cur_id = id;
		...
		vlcJS.playlistActive && ( vlcJS.playlistActive.className = "active" );
	},
	...
	pl_pause : function() {
		if( this.state == "playing" )
			...
			loadXMLDoc( 'requests/status.xml?command=pl_play', parse_status );
	},
	...
	slider_seek : function() {
		//this.moving = true;
		//console.log( piecelength / ( 100 / parseInt( document.getElementById('scrub').style.left ) ) );
		this.seek( ( piecelength / ( 100 / parseInt( document.getElementById('scrub').style.left ) ) ) >> 0 );
	},
	...
	seek : function( pos ) {
		this.moving = true;
		//console.log( pos );
		//console.log( 'requests/status.xml?command=seek&val='+pos );
...
function parse_status()
{
    ...
    {
        if( req.status == 200 )
        {	//console.log( req.responseXML.URL );
			//if( req.responseXML.URL.indexOf( 'status.xml' ) === -1 )
			//	return false;
			...
			piecelength = status.getElementsByTagName( 'length' )[0].firstChild.data / 1;
			
			//console.log( positiontag );
			if( !vlcJS.moving )
				document.getElementById('scrub').style.left = ( positiontag[0].firstChild.data * 100 ) + "%";
			else
				vlcJS.moving = false;
